...
| **Review Mode / Extension** | The primary maintainer review mode for Oh My Pi in `image/extension/bluefin-review/`. It provides the queue rail, dashboard, pipeline trace, companion agents (`bluefin-doctrine`, `bluefin-reviewer`, `bluefin-security`, `bluefin-correctness`, `bluefin-test-coverage`, `bluefin-simplicity`, `bluefin-ci-triage`), and inspection tools. | OMP owns execution, session, and tool boundaries. |
