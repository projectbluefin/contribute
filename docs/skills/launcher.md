...
- A loopback endpoint gets `--network slirp4netns:allow_host_loopback=true`
  ...
  through unchanged and gets no network mode of its own. Do not publish a
  port, share the host network namespace, or start a listener — this appliance
  is a client of one endpoint, and peer aggregation is llmman's own feature,
  configured in llmman.
- `OPENAI_BASE_URL` crosses by value (an address, and an auditable one);
  ...
  the operator's cloud OpenAI key.
...
- OMP still owns model choice. `llmman_model` is what `doctor` verifies and
  what the launch banner names; it is not passed as a model selection.
