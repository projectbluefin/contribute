...
# -----------------------------------------------------------------------------
# Scenarios 8-10: an explicitly selected local llmman endpoint.
#
# Bluefin's Agent Mode binds llmman to the host loopback, which an isolated
# container cannot reach. The appliance may offer to donate that inference only
# when the operator names the endpoint themselves, and only without loosening
# anything else: no host home mount, no wider credential allowlist, no key on a
# command line, and no listener of our own.
# -----------------------------------------------------------------------------
...
# The default. Nothing about the cloud-provider path may change until someone
# writes an endpoint into the config file.
test_local_inference_off_by_default() {
  ...
  assert_not_contains "$run_cmd" "OPENAI_BASE_URL" "no base URL without an explicit selection"
  assert_not_contains "$run_cmd" "--network" "no network mode is chosen without an explicit selection"
  assert_contains "$run_cmd" "--env OPENAI_API_KEY" "the configured cloud key is still forwarded by name"
  assert_contains "$(cat "$podman_env_log")" "OPENAI_API_KEY=not-a-real-cloud-openai-key" \
    "the cloud key reaches the container unchanged"
  assert_not_contains "$output" "local inference" "no local-inference banner without a selection"

  local doctor_out
  doctor_out="$("$launcher" doctor)"
  assert_contains "$doctor_out" "not selected" "doctor names local inference as unselected"
  assert_contains "$doctor_out" "checks passed, 0 failed" "an unselected endpoint is not a failure"
  assert_eq "$(cat "$curl_log")" "" "doctor must not probe an endpoint nobody selected"
}

# The opt-in, against the loopback daemon Agent Mode actually provisions.
...
llmman_model: qwen3-coder-30b"
  ...
  # A cloud key the operator also has: it must not be what reaches their own
  # daemon, and it must not be what the agent authenticates with here.
  export OPENAI_API_KEY="not-a-real-cloud-openai-key"
  ...
  assert_contains "$run_cmd" "--network slirp4netns:allow_host_loopback=true" "documented loopback transport"
  assert_contains "$run_cmd" "--env OPENAI_BASE_URL=http://10.0.2.2:17434/v1" "host loopback rewritten for the container"
  assert_contains "$run_cmd" "--env OPENAI_API_KEY" "llmman key forwarded by name"
  assert_contains "$output" "local inference: http://10.0.2.2:17434/v1" "launch names the endpoint in use"
  assert_contains "$output" "select qwen3-coder-30b in OMP" "launch names the model without choosing it"

  # End to end: what Podman would hand the container is the llmman key, not the
  # operator's cloud key and not a value from anyone's argv.
